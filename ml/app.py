...
print(f"Using Keras {keras.__version__} for model loading")
...
#constants
GOOGLE_MAPS_API_KEY = "AIzaSyDj-iafVtI7FY83vw2I_EqV0-CpSqx_I3Y"
...
def enhance_with_rules(text: str, model_symptoms: List[str], symptoms_with_conf: List[Dict]) -> Tuple[List[str], List[Dict]]:
    ...
    non_medical = {
        ...
        'ate', 'playing', 'game', 'working', 'feeling'
    }
...
@app.get("/symptoms")
def list_symptoms():
    """List all symptoms in knowledge base"""
...
@app.post("/nearby_medical_help")
async def nearby_medical_help(request: dict):
    """Find nearby hospitals or clinics based on severity"""
    try:
        lat = request.get("lat")
        lng = request.get("lng")
        ...
        if severity == "emergency":
            ...
            keyword = "clinic doctor medical center"
        ...
        return {
            ...
            "severity": severity,
            "search_type": "hospitals" if severity == "emergency" else "clinics"
        }
        
    except Exception as e:
        print(f"Error finding nearby medical help: {str(e)}")
