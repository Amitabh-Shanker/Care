...
import { saveTextAnalysis } from "@/lib/analysisService";
...
const TextSymptomInput = ({ onAnalysis, isAnalyzing, setIsAnalyzing }: TextSymptomInputProps) => {
  ...
  const handleAnalyze = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      ...
      // Save to text_analyses table
      const { success, error: saveError } = await saveTextAnalysis(symptoms, data);
      if (!success) {
        console.error("Error saving text analysis:", saveError);
      }
